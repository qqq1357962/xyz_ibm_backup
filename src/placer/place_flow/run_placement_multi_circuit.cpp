...
void run_placement_main_multi_circuit() {
    ...
    rawdb->bot_Max_util = data.maxUtilM[0].item<double>();
    rawdb->top_Max_util = data.maxUtilM[1].item<double>();
    if (st::setting.transfer_txt) {

        rawdb->writeDEF2ICCAD2022(st::setting.output_path + "_contest.txt");
    } else {
        ...
        if (false) {
            ...
            rawdb->write_openroad_partition(st::setting.load_def_template, output_def, node_selected);
            output_def = st::setting.output_path + "_partition_1.def";
        ...
        if (st::setting.gp) {
            ...
            int is_move_macro = false;
            int is_init_macro = false;
            int is_init_stdcell = false;

...
void myLog(torch::Tensor x)
{
    cout<<x.view({-1}).slice(0, 0, 100)<<endl;
}
...
torch::Tensor density_map_forward(torch::Tensor normalize_node_info,
    ...
    return density_map_cuda_forward(normalize_node_info, sorted_node_map, aux_mat, num_bin_x, num_bin_y, num_nodes, deterministic);
}
...
torch::Tensor macro_density_map_forward(torch::Tensor normalize_node_info,
    ...
    return macro_density_map_cuda_forward(normalize_node_info, sorted_node_map, aux_mat, num_bin_x, num_bin_y, num_nodes, deterministic);
}
...
torch::Tensor density_backward(torch::Tensor normalize_node_info,
    ...
    return density_backward_cuda(normalize_node_info, sorted_node_map, density_map, num_bin_x, num_bin_y, num_nodes);
}
...
torch::Tensor apply_kernel(torch::Tensor density_map,
    ...
    return apply_kernel_cuda(density_map, aux_mat, num_bin_x, num_bin_y, kernel, kernelWidth);
}
...
tuple<at::Tensor, at::Tensor> torch_dct_idct(
    at::Tensor density_map, tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> fft_scale) {
    auto [potential_scale, potential_coeff, force_x_scale, force_y_scale, force_x_coeff, force_y_coeff] = fft_scale;

    at::Tensor fft_coeff = 4 * dct_2d(density_map);  // FIXME:

    // Real number, M x N
    at::Tensor potential_map = real(idct_2d(fft_coeff * potential_scale)) * potential_coeff;  // M x N
    ...
    at::Tensor grad_mat = torch::vstack({force_x_map.unsqueeze(0), force_y_map.unsqueeze(0)});  // 2 x M x N
...
at::Tensor compute_electronic_force(at::Tensor x, at::Tensor scale, at::Tensor coeff, int dim) {
    ...
    if (dim == 0)
        ...
        x = torch::cat({x.index({"...", Slice(None, 1)}) * 0, x.index({"...", Slice(1, None)}).flip({1})}, 1);

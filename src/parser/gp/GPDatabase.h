...
protected:
    ...
    std::tuple<index_type, index_type, index_type> ori_db_info = {
        std::numeric_limits<index_type>::max(),
        std::numeric_limits<index_type>::max(),
        std::numeric_limits<index_type>::max()};  // ori_net_id, ori_node_id, ori_node_pin_id
};
...
class GPGridGraph {
public:
    const std::vector<std::vector<coord_type>>& gcellGrid() const { return gcell_grid; }
    std::vector<std::vector<coord_type>>& gcellGrid() { return gcell_grid; }
    const std::vector<std::vector<int>>& numTracks() const { return num_tracks; }
    std::vector<std::vector<int>>& numTracks() { return num_tracks; }
...
protected:
    // 2D Vector, first dimension is for layers, second is for grids
    std::vector<std::vector<int>> num_tracks;
    // 2D Vector, first dimension is for track direction, second is for grids
    // track direction (0 for X / Vertical Line, 1 for Y / Horizontal Line)
    std::vector<std::vector<coord_type>> gcell_grid;
};
...
protected:
    ...
    // track direction (0 for X / Vertical Line, 1 for Y / Horizontal Line)
    std::vector<int> layerTrackDirection;
...
public:
    ...
    std::pair<index_type, index_type> transPos2GridIdx(const point_type& pos) const;
    void updateNodePinRelativePos(const GPNode& node, orient_type& origOrient, orient_type& newOrient);  //@@@ TODO
    ...
    void updateVarByDieId(int dieId);  // ICCAD 2022
    ...
    const int getNumRow() const { return num_rows; }
    ...
    torch::Tensor getNodeOrientTensor();
    torch::Tensor getNodeTypeTensor();
    
    std::vector<torch::Tensor> getNode2PinInfoTensor();  // node_id2pin_id_list, node2pin_list_end
...
public:
    ...
    /* bondings */
    int num_bondings;
    std::vector<GPBonding> bondings;  // store all nodes Mov + FloatMov + Fix + IOPin + Blkg + FloatIOPin + FloatFix
    int bondingSizeX;
    int bondingSizeY;
    int bondingCost;
    int bondingSpacing;
    torch::Tensor getBondingCPosTensor();
    torch::Tensor getBondingSizeTensor();

    /* multi lib */
    void addPin_mT(db::Pin* dbpin, const db::PinType* pintype, GPNode& node, GPNet& net, bool isIOPin);
    std::vector<GPNode> nodes_mT;  // store all nodes Mov + FloatMov + Fix + IOPin + Blkg + FloatIOPin + FloatFix
    std::vector<GPPin> pins_mT;    // store all pins
    std::vector<GPNet> nets_mT;    // store all nets
    torch::Tensor getPinRelCPosTensor_mT();  // pin_cx - node_cx
    torch::Tensor getNodeSizeTensor_mT();
    torch::Tensor getPinSizeTensor_mT();
    torch::Tensor getNodeOrientTensor_mT();
};

...
bool Database::readDEFPG(const std::string& file) {
    ...
    for (std::size_t netId = 0; netId < net_num; netId++) {
        bondings.emplace_back(
            std::numeric_limits<int>::min(), std::numeric_limits<int>::min(), db->sites[0].width(), db->sites[0].height(), netId);
    }

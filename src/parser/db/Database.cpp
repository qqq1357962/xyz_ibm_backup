...
void Database::load() {
    ...
    if (rawDBArgs.BookshelfAux != "" && rawDBArgs.BookshelfPl != "") {
        rawDBArgs.Format = "bookshelf";
    ...
    if (rawDBArgs.ICCAD2022InputFile != "") {
        rawDBArgs.Format = "iccad2022";
        readICCAD2022(rawDBArgs.ICCAD2022InputFile);
    }
...
void Database::save(const std::string& given_prefix) {
    ...
    if (given_prefix != "") {
        ...
        if (rawDBArgs.Format == "bookshelf") {
            filename += ".pl";
        } else if (rawDBArgs.Format == "lefdef") {
            filename += ".def";
        } else if (rawDBArgs.Format == "iccad2022") {
            filename += ".txt";
        }
    } else {
        filename = rawDBArgs.OutputFile;
    }

    if (rawDBArgs.Format == "bookshelf" && filename != "") {
        writeBSPl(filename);
    } else if (rawDBArgs.Format == "lefdef" && filename != "") {
        writeICCAD2017(rawDBArgs.DefFile, filename);
    } else if (rawDBArgs.Format == "iccad2022" && filename != "") {
        writeICCAD2022(filename);
    } else {
        printlog(LOG_ERROR, "Cannot save placement! filename: %s format: %s", filename.c_str(), rawDBArgs.Format.c_str());
    }
}
...
void Database::SetupGRGrid() {
    ...
    if (!numGCellX) {
        gcellTrackX = rawDBArgs.customGcellTrackX;
        numGCellX = (numTracksX - 1) / gcellTrackX + 1;
        stepGCellX = minPitchV * gcellTrackX;
        // FIXME:
        // printlog(LOG_INFO,
        //          "Can't detect GcellGrid X in DEF, set gcellTrackX=%d numGCellX=%u step=%d",
        //          gcellTrackX,
        //          numGCellX,
        //          stepGCellX);
    }
    ...
    if (!numGCellY) {
        gcellTrackY = rawDBArgs.customGcellTrackY;
        numGCellY = (numTracksY - 1) / gcellTrackY + 1;
        stepGCellY = minPitchH * gcellTrackY;
        // FIXME:
        // printlog(LOG_INFO,
        //          "Can't detect GcellGrid Y in DEF, set gcellTrackY=%d numGCellY=%u step=%d",
        //          gcellTrackY,
        //          numGCellY,
        //          stepGCellY);
    }
    ...
    printlog(LOG_VERBOSE, "set up tracks begin");
    for (unsigned i = 0; i != numLayers; ++i) {
        Layer* layer = getRLayer(i);
        Track& track = layer->track;
        int start, step, end;
    ...
    printlog(LOG_VERBOSE, "set up tracks done");
    ...
    printlog(LOG_VERBOSE, "set up GCell supply start");
    ...
    printlog(LOG_VERBOSE, "set up GCell supply done (%ld)", loopCount);
}

void Database::setup() {
    if (rawDBArgs.Format == "iccad2022") return;
    SetupLayers();
    SetupCellLibrary();
    SetupFloorplan();
    SetupRegions();

...
public:
    ...
    void addType(CellType* celltype);
    CellType * getType(int id);
    ...
    int typeCount() const { return _type.size(); }
    void typeClone(CellType* type){
        _type.clear();
        for(int i = 0; i < type->typeCount(); i++){
            _type.push_back(type->getType(i));
        }
    };
...
public:
    ...
    int gpdb_id = -1;
    int celltypeId = -1; // ICCAD2022: celltypeId
    int id = -1;
    ...
    int getDieId() const { return _dieId; }
    void setDieId(int dieId_) { _dieId = dieId_; }
    void setOrient(int orient) { _orient = orient;}
    ...
    void place(int x, int y, int dieId);
    void place_with_orient(int x, int y, int dieId, int cell_orient);

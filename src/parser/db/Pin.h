...
public:
    ...
    void setBounds(const int lx, const int ly, const int hx, const int hy);
...
public:
    ...
    int gpdb_id = -1;
    int pin_id_in_parent = -1;  // pin id in its parent
    bool is_connected = false;
    string name;
    ...
    Pin(Cell* cell, int i) : cell(cell), type(cell->ctype()->pins[i]), name(cell->ctype()->pins[i]->name()) {
        pin_id_in_parent = i;
    }
    Pin(IOPin* iopin) : iopin(iopin), type(iopin->type) { pin_id_in_parent = 0; }

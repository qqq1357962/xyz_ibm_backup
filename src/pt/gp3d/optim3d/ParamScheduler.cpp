...
ParamScheduler::ParamScheduler(NodeData3D& data) : recorder(_metrics) {
    ...
    node_size = data.node_size.clone().to(torch::kCPU);
    ...
    density_weight_xy = st::setting.density_weight;
    ...
    // base_gamma = st::setting.wa_coeff * torch::sum(data.unit_len.index({2}) / 50).item<float>();

    wa_coeff = 10 * base_gamma;
    net_weight_coef = st::setting.net_weight_coef;
    force_coeff = st::setting.force_coeff_2d;
    density_weight_local = torch::ones({mov_node_to_num_pins.size(0)}, dtype(torch::kFloat).device(data.device));

    logger.info("Placer model info, [Net coef: %.2f], [force coef: %.2f]", 
                    net_weight_coef, force_coeff);

    mu = 1.0;
    max_life = 30;
    life = max_life;
    stop_overflow = st::setting.stop_overflow_3d;
    // if (st::setting.force_coeff_2d == 0){
    //     stop_overflow *= 2.2;
    // }
...
void ParamScheduler::set_init_param(double init_density_weight) {
    __init_density_weight__ = init_density_weight;
    density_weight = density_weight * init_density_weight;
    density_weight_xy = density_weight;
    update_precond_weight();
}

void ParamScheduler::push_metric(double hpwl, double hpwl_xy, double overflow, int iteration) {//@@
    const unordered_map<string, double> metrics_dict = {
        ...
        {"net_weight_coef", net_weight_coef},
        {"iteration", iteration}
    };
...
bool ParamScheduler::update_best_sol(torch::Tensor sol) {
    ...
    //     return update_flag;
    // }
    if (iter < st::setting.min_gp3d_record_step) {
        return update_flag;
    }
...
bool ParamScheduler::need_to_early_stop() {
    ...
    if ((st::setting.wa_z_model == "z") && (ptr_ovfl < stop_overflow)) return true;
    if (iter < min_stop_iter) return false;

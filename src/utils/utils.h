...
#include "draw_placement/draw_placement.h"
#include "geo.h"
#include "log.h"
...
#ifdef WITH_OPENCV
inline cv::Mat tensor2Mat(torch::Tensor &i_tensor) {
    int h = i_tensor.size(0), w = i_tensor.size(1), d = 1;
    i_tensor = i_tensor.to(torch::kCPU);
    cv::Mat o_Mat(cv::Size(w, h), CV_32F, i_tensor.data_ptr());
    return o_Mat;
}
#endif // WITH_OPENCV

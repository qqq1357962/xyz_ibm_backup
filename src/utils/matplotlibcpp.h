...
typedef std::map<std::string, std::variant<int, double, float, std::string>> plt_kwargs;

inline void PyDict_SetVariant(PyObject* kwargs, const plt_kwargs& keywords) {
    for (const auto& [k, v] : keywords) {
        std::visit(
            [&kwargs, &k](auto&& arg) {
                using T = std::decay_t<decltype(arg)>;
                if constexpr (std::is_same_v<T, std::string>) {
                    PyDict_SetItem(kwargs, PyUnicode_FromString(k.c_str()), PyUnicode_FromString(arg.c_str()));
                } else if constexpr (std::is_same_v<T, int>) {
                    PyDict_SetItem(kwargs, PyUnicode_FromString(k.c_str()), PyFloat_FromDouble((double)arg));
...
inline PyObject* imshow(void *ptr, const NPY_TYPES type, const int rows, const int columns, const int colors, const std::map<std::string, std::string> &keywords, PyObject** out)
{
...
#ifdef WITH_OPENCV
inline PyObject* imshow(const cv::Mat &image, const std::map<std::string, std::string> &keywords = {})
{
    ...
    return detail::imshow(image2.data, npy_type, image2.rows, image2.cols, image2.channels(), keywords, nullptr);
}
#endif // WITH_OPENCV
#endif // WITHOUT_NUMPY
...
bool quiver(const std::vector<NumericX>& x, const std::vector<NumericY>& y, const std::vector<NumericZ>& z, const std::vector<NumericU>& u, const std::vector<NumericW>& w, const std::vector<NumericV>& v, const plt_kwargs& keywords = {})
{
...
//       PyDict_SetItemString(kwargs, it->first.c_str(), PyUnicode_FromString(it->second.c_str()));
//   }
  PyDict_SetVariant(kwargs, keywords);

...
bool hannanLegalize(DetailedPlaceData& db,
    ...
    for (int iter = 0; iter < max_iters; ++iter) {
        ...
        spacing_x = std::min(spacing_x, (db.xh - db.xl) / 512);
        spacing_y = std::min(spacing_y, (db.yh - db.yl) / 512);
        ...
        for (auto node_id : macros) {
            ...
            for (int ii=0;ii<search_grids.size();ii++) {
                auto grid_offset=search_grids[ii];
                ...
                if (ix < grid_map.dim_x() && iy < grid_map.dim_y()) {
                    ...
                    float aligned_yl = db.align2row_withFP(yl, height);
                    if (aligned_xl < xl) {

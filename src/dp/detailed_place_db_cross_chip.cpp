#include "detailed_place_db_cross_chip.h"

namespace dpc {

int floorDiv(float a, float b) { return std::floor(a / b); }

int ceilDiv(float a, float b) { return std::ceil(a / b); }

int floorDivRound(float a, float b, int prec) { return std::floor(std::round(a * prec) / std::round(b * prec)); }

int ceilDivRound(float a, float b, int prec) { return std::ceil(std::round(a * prec) / std::round(b * prec)); }

int roundDiv(float a, float b) { return std::round(a / b); }

DetailedPlaceDataTensor::DetailedPlaceDataTensor(NodeData& data, ViaData& via_data, torch::Tensor node_pos_,
                                                 torch::Tensor exact_node_size_) {
    /* process pos c->l */
    node_pos_init = node_pos_.clone();
    exact_node_size = exact_node_size_;

    auto exact_node_size_x = exact_node_size.index({"...", 0});
    auto exact_node_size_y = exact_node_size.index({"...", 1});
    node_size_x = exact_node_size_x;
    node_size_y = exact_node_size_y;

    /* node pos */
    init_x = node_pos_init.index({"...", 0}).clone();  // FIXME: init -> center
    init_y = node_pos_init.index({"...", 1}).clone();
    x = node_pos_init.index({"...", 0}) - exact_node_size_x / 2;
    y = node_pos_init.index({"...", 1}) - exact_node_size_y / 2;
    // x = init_x.clone();  // FIXME: center pos -> lower-left
    // y = init_y.clone();

    /* process node sizes */
    auto via_node_size = exact_node_size.index({Slice(data.cell_mov_rhs, None)});
    auto node_size_bot = torch::cat({data.node_size_bot, via_node_size}, 0);
    auto node_size_top = torch::cat({data.node_size_top, via_node_size}, 0);
    node_sizes.push_back(node_size_bot);
    node_sizes.push_back(node_size_top);
    node_size_xs.push_back(node_size_bot.index({"...", 0}));
    node_size_xs.push_back(node_size_top.index({"...", 0}));
    node_size_ys.push_back(node_size_bot.index({"...", 1}));
    node_size_ys.push_back(node_size_top.index({"...", 1}));

    /* preprocess pin_pos */
    pin_rel_cpos = data.pin_rel_cpos.clone();
    torch::Tensor via_pin_rel_cpos = torch::zeros({data.num_nets, 2}, torch::dtype(node_pos_.dtype()));
    torch::Tensor pin_rel_lpos_bot = data.pin_rel_cpos_bot.clone();
    torch::Tensor pin_rel_lpos_top = data.pin_rel_cpos_top.clone();
    pin_rel_lpos_bot = torch::cat({pin_rel_lpos_bot, via_pin_rel_cpos}, 0);
    pin_rel_lpos_top = torch::cat({pin_rel_lpos_top, via_pin_rel_cpos}, 0);

    torch::Tensor pin_rel_lpos = data.pin_rel_cpos.clone();
    for (int64_t i = 0; i < data.num_pins; i++) {
        int64_t node_id = data.pin_id2node_id[i].item<int64_t>();
        pin_rel_lpos_bot[i] += node_size_bot[node_id] / 2;
        pin_rel_lpos_top[i] += node_size_top[node_id] / 2;
        pin_rel_lpos[i] += exact_node_size[node_id] / 2;
    }
    pin_rel_lposes.push_back(pin_rel_lpos_bot);
    pin_rel_lposes.push_back(pin_rel_lpos_top);

    pin_offset_x = pin_rel_lpos.index({"...", 0});
    pin_offset_y = pin_rel_lpos.index({"...", 1});
    pin_offset_xs.push_back(pin_rel_lpos_bot.index({"...", 0}));
    pin_offset_xs.push_back(pin_rel_lpos_top.index({"...", 0}));
    pin_offset_ys.push_back(pin_rel_lpos_bot.index({"...", 1}));
    pin_offset_ys.push_back(pin_rel_lpos_top.index({"...", 1}));

    /* cell number info */
    num_pins = data.num_pins;
    int num_nets = data.num_nets;
    int num_nodes = data.num_nodes;

    /* update node -> pin map */
    torch::Tensor via_node2pin_list_end =
        torch::arange(num_pins + 1, num_pins + num_nets + 1, torch::dtype(torch::kInt64));
    torch::Tensor node2pin_list_end = torch::cat({data.node2pin_list_end, via_node2pin_list_end}, 0);
    flat_node2pin_start_map = torch::cat({torch::zeros({1}, torch::dtype(torch::kInt64)), node2pin_list_end}, 0);
    torch::Tensor via_node2pin_list = torch::arange(num_pins, num_pins + num_nets, torch::dtype(torch::kInt64));
    flat_node2pin_map = torch::cat({data.node2pin_list, via_node2pin_list}, 0);
    pin2node_map = data.pin_id2node_id.clone();

    /* update net -> pin map / hyperedge_list*/
    flat_net2pin_start_map = torch::cat({torch::zeros({1}, torch::dtype(torch::kInt64)), data.hyperedge_list_end}, 0);
    flat_net2pin_map = data.hyperedge_list;
    pin2net_map = data.pin_id2net_id.clone();

    net_mask = torch::ones(num_nets, torch::dtype(torch::kInt));

    /* create via map */
    via_site_width = (data.bondingInfo[0] + data.bondingInfo[2]);
    via_site_height = (data.bondingInfo[1] + data.bondingInfo[2]);
    // this also considers cell width which is not integral times of site_width
    int num_via_x = torch::floor((via_data.core_info[1] - via_data.core_info[0]) / via_site_width).item<int>();
    // FIXME:
    int num_via_y = via_data.numRows.item<int>();
    node_weight = data.node_die.clone();

    via_map = -torch::ones({num_via_x, num_via_y}, torch::kInt);
    logger.info("Generating via map (%d, %d)", num_via_x, num_via_y);
    via_index = -torch::ones({num_nets, 2}, torch::kInt);
    float row_shift = via_data.core_info[0].item<float>();
    // FIXME:

    for (int i = 0; i < num_nets; i++) {
        int64_t idx = i + num_nodes;
        if (node_weight[idx].item<int>() == 2) {
            int x_id = torch::floor((node_pos_init[idx][0] - row_shift)/ via_site_width).item<int>();
            int y_id = torch::floor((node_pos_init[idx][1] - row_shift)/ via_site_height).item<int>();
            via_map[x_id][y_id] = idx;
            via_index[i][0] = x_id;
            via_index[i][1] = y_id;
            node_weight[idx] = 1;  // FIXME: for 0/1 switch
        } else {
            node_weight[idx] = 0;  // FIXME: for 0/1 switch
        }
    }
}

DetailedPlaceData::DetailedPlaceData(NodeData& data, DetailedPlaceDataTensor& at_db, ViaData& via_data)
    : x(at_db.x.accessor<float, 1>()),
      y(at_db.y.accessor<float, 1>()),
      init_x(at_db.init_x.accessor<float, 1>()),
      init_y(at_db.init_y.accessor<float, 1>()),
      node_size_x(at_db.node_size_x.accessor<float, 1>()),
      node_size_y(at_db.node_size_y.accessor<float, 1>()),
      pin_offset_x(at_db.pin_offset_x.accessor<float, 1>()),
      pin_offset_y(at_db.pin_offset_y.accessor<float, 1>()),
      flat_node2pin_start_map(at_db.flat_node2pin_start_map.accessor<int64_t, 1>()),
      flat_node2pin_map(at_db.flat_node2pin_map.accessor<int64_t, 1>()),
      pin2node_map(at_db.pin2node_map.accessor<int64_t, 1>()),
      flat_net2pin_start_map(at_db.flat_net2pin_start_map.accessor<int64_t, 1>()),
      flat_net2pin_map(at_db.flat_net2pin_map.accessor<int64_t, 1>()),
      pin2net_map(at_db.pin2net_map.accessor<int64_t, 1>()),
      net_mask(at_db.net_mask.accessor<int, 1>()),
      node_weight(at_db.node_weight.accessor<int, 1>()),
      via_map(at_db.via_map.accessor<int, 2>()),
      via_index(at_db.via_index.accessor<int, 2>()) {
    /* construct dp info form placedb */
    row_width = (via_data.core_info[1] - via_data.core_info[0]).item<float>();
    // site_width = 1;                                 // FIXME: pre_scale by site_width
    site_width_safe_divide = int(1);  // TODO:
    row_shift = via_data.core_info[0].item<float>();
    via_site_width = (data.bondingInfo[0] + data.bondingInfo[2]).item<float>();
    via_site_height = (data.bondingInfo[1] + data.bondingInfo[2]).item<float>();

    num_threads = std::max(st::setting.num_threads, 1);
    num_nodes = at_db.node_pos_init.size(0);  // FIXME: #nodes = cell+via #node
    num_movable_nodes = data.num_nodes;       // FIXME: #mov_nodes = cell #node
    num_nets = data.num_nets;
    num_pins = data.num_pins;

    xl = data.die_info[0].item<float>();
    xh = data.die_info[1].item<float>();
    yl = data.die_info[2].item<float>();
    yh = data.die_info[3].item<float>();

    node_size_xs.push_back(at_db.node_size_xs[0].accessor<float, 1>());
    node_size_xs.push_back(at_db.node_size_xs[1].accessor<float, 1>());
    node_size_ys.push_back(at_db.node_size_ys[0].accessor<float, 1>());
    node_size_ys.push_back(at_db.node_size_ys[1].accessor<float, 1>());

    pin_offset_xs.push_back(at_db.pin_offset_xs[0].accessor<float, 1>());// die 0
    pin_offset_xs.push_back(at_db.pin_offset_xs[1].accessor<float, 1>());// die 1
    pin_offset_ys.push_back(at_db.pin_offset_ys[0].accessor<float, 1>());
    pin_offset_ys.push_back(at_db.pin_offset_ys[1].accessor<float, 1>());

    // debug --> hpwl_kernel info
    pin_id2node_id = data.pin_id2node_id;
    pin_rel_cpos = at_db.pin_rel_cpos;
    pin_rel_cpos_bot = data.pin_rel_cpos_bot.clone();
    pin_rel_cpos_top = data.pin_rel_cpos_top.clone();

    nodes = via_data.nodes;
    nets = via_data.nets;
    int num_swappable_node = 0;
    node_swap = torch::zeros(num_movable_nodes, torch::dtype(torch::kInt));
    torch::Tensor node_net_size_count = torch::zeros({num_movable_nodes, 1}, torch::dtype(torch::kInt));
    for (int i = 0; i != num_movable_nodes; i++) {
        auto node = nodes[i];
        int num_nets_node = node->Nets.size();
        for (unsigned n = 0; n != num_nets_node; ++n) {
            int net_id = node->Nets[n]->id;
            if (data.net_to_num_pins[net_id].item<int>() >= 100) {
                node_net_size_count[i] += 1;
            }
        }
    }
    node_swap = torch::_cast_Int(node_net_size_count == 0);
    logger.info("%d/%d cells are only connected with low-pin nets", node_swap.sum().item<int>(), num_movable_nodes);
    logger.info("#node: %d, #mov_node: %d", num_nodes, num_movable_nodes);
}  // END MODULE

//---------------------------------------------------------------------
// TODO:

void DetailedPlaceData::make_row2node_map(const torch::TensorAccessor<float, 1> vx,
                                          const torch::TensorAccessor<float, 1> vy,
                                          std::vector<std::vector<std::vector<int>>>& row2node_maps) {
    // distribute cells to rows
    for (int i = 0; i < num_movable_nodes; ++i) {  // FIXME: exclude vias
        int c_id = node_weight[i];

        float node_yl = vy[i];
        float node_yh = node_yl + node_size_ys[c_id][i];

        // FIXME: float divide
        // int row_idxl = floorDiv(node_yl - yl, row_height);
        int row_idxl = floorDivRound(node_yl - yl, row_heights[c_id], site_width_safe_divide);
        // int row_idxh = ceilDiv(node_yh - yl, row_height);
        int row_idxh = ceilDivRound(node_yh - yl, row_heights[c_id], site_width_safe_divide);

        row_idxl = std::max(row_idxl, 0);
        row_idxh = std::min(row_idxh, num_sites_ys[c_id]);

        for (int row_id = row_idxl; row_id < row_idxh; ++row_id) {
            float row_yl = yl + row_id * row_heights[c_id];
            float row_yh = row_yl + row_heights[c_id];

            if (node_yl < row_yh && node_yh > row_yl)  // overlap with row
            {
                row2node_maps[c_id][row_id].push_back(i);
            }
        }
    }

    for (int c_id = 0; c_id < 2; c_id++) {
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, 1)
        for (int i = 0; i < num_sites_ys[c_id]; ++i) {
            auto& row2nodes = row2node_maps[c_id][i];
            std::sort(row2nodes.begin(), row2nodes.end(), [&](int node_id1, int node_id2) {
                float x1 = vx[node_id1];
                float x2 = vx[node_id2];
                return x1 < x2 || (x1 == x2 && node_id1 < node_id2);
            });
            // After sorting by left edge,
            // there is a special case for fixed cells where
            // one fixed cell is completely within another in a row.
            // This will cause failure to detect some overlaps.
            // We need to remove the "small" fixed cell that is inside another.
            if (!row2nodes.empty()) {
                std::vector<int> tmp_nodes;
                tmp_nodes.reserve(row2nodes.size());
                tmp_nodes.push_back(row2nodes.front());
                for (int j = 1, je = row2nodes.size(); j < je; ++j) {
                    int node_id1 = row2nodes.at(j - 1);
                    int node_id2 = row2nodes.at(j);
                    // two fixed cells
                    if (node_id1 >= num_movable_nodes && node_id2 >= num_movable_nodes) {
                        float xl1 = vx[node_id1];
                        float xl2 = vx[node_id2];
                        float width1 = node_size_x[node_id1];
                        float width2 = node_size_x[node_id2];
                        float xh1 = xl1 + width1;
                        float xh2 = xl2 + width2;
                        // only collect node_id2 if its right edge is righter than node_id1
                        if (xh1 < xh2) {
                            tmp_nodes.push_back(node_id2);
                        }
                    } else {
                        tmp_nodes.push_back(node_id2);
                    }
                }
                row2nodes.swap(tmp_nodes);

                // sort according to center
                std::sort(row2nodes.begin(), row2nodes.end(), [&](int node_id1, int node_id2) {
                    float x1 = vx[node_id1] + node_size_x[node_id1] / 2;
                    float x2 = vx[node_id2] + node_size_x[node_id2] / 2;
                    return x1 < x2 || (x1 == x2 && node_id1 < node_id2);
                });
            }
        }
    }
}  // END MODULE

//---------------------------------------------------------------------

void DetailedPlaceData::make_bin2node_map(const torch::TensorAccessor<float, 1> host_x,
                                          const torch::TensorAccessor<float, 1> host_y,
                                          const torch::TensorAccessor<float, 1> host_node_size_x,
                                          const torch::TensorAccessor<float, 1> host_node_size_y,
                                          std::vector<std::vector<std::vector<int>>>& bin2node_maps,
                                          std::vector<BinMapIndex>& node2bin_map) {
    // construct bin2node_maps
    for (int i = 0; i < num_movable_nodes; ++i) {
        // if (node_weight[i] == 0) continue;
        int c_id = node_weight[i];

        int node_id = i;
        float node_x = host_x[node_id] + host_node_size_x[node_id] / 2;
        float node_y = host_y[node_id] + host_node_size_y[node_id] / 2;

        int bx = std::min(std::max((int)floorDiv(node_x - xl, bin_size_x), 0), num_bins_x - 1);
        int by = std::min(std::max((int)floorDiv(node_y - yl, bin_size_y), 0), num_bins_y - 1);
        int bin_id = bx * num_bins_y + by;
        // bin2node_map.at(bin_id).push_back(node_id);
        bin2node_maps[c_id].at(bin_id).push_back(node_id);
    }
    // construct node2bin_map
    for (int c_id = 0; c_id < 2; c_id++) {
        for (unsigned int bin_id = 0; bin_id < bin2node_maps[c_id].size(); ++bin_id) {
            for (unsigned int sub_id = 0; sub_id < bin2node_maps[c_id][bin_id].size(); ++sub_id) {
                int node_id = bin2node_maps[c_id][bin_id][sub_id];
                BinMapIndex& bm_idx = node2bin_map.at(node_id);
                bm_idx.bin_id = bin_id;
                bm_idx.sub_id = sub_id;
                bm_idx.chip_id = c_id;
            }
        }
    }
}  // END MODULE

//--------------------------------------------------------------------

float DetailedPlaceData::compute_total_hpwl() {
    partial_hpwl = torch::zeros({num_nets, 2}, torch::dtype(torch::kFloat));
    float total_hpwl = 0;
    for (int net_id = 0; net_id < num_nets; ++net_id) {
        total_hpwl += compute_net_hpwl(net_id);
    }
    return total_hpwl;
}

//--------------------------------------------------------------------

float DetailedPlaceData::compute_total_hpwl_concurrent(const torch::TensorAccessor<float, 1> x,
                                                       const torch::TensorAccessor<float, 1> y, float* net_hpwls) {
    partial_hpwl = torch::zeros({num_nets, 2}, torch::dtype(torch::kFloat));
#pragma omp parallel for num_threads(num_threads)
    for (int i = 0; i < num_nets; ++i) {
        net_hpwls[i] = compute_net_hpwl(i);
    }
    float hpwl = 0;
    // I found OpenMP reduction cannot guarantee run-to-run determinism
    // #pragma omp parallel for num_threads(state.num_threads) default(shared)
    // reduction(+:hpwl)
    for (int i = 0; i < num_nets; ++i) {
        hpwl += net_hpwls[i];
        // cout << "Net " << i << " " << net_hpwls[i] << endl;
        // cout << partial_hpwl[i][0] << endl;
        // cout << partial_hpwl[i][1] << endl;
    }

    // printf("Torch calculated HPWL: %.2f\n", torch::sum(partial_hpwl.sum(1)).item<float>());
    return hpwl;
}  // END MODULE

//---------------------------------------------------------------------

float DetailedPlaceData::compute_net_hpwl(int net_id) const {
    vector<Box> boxs;
    boxs.emplace_back(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                      -std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());
    boxs.emplace_back(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                      -std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());

    for (int c_id = 0; c_id < 2; c_id++) {
        bool node_exist_flag = false;
        for (int net2pin_id = flat_net2pin_start_map[net_id]; net2pin_id < flat_net2pin_start_map[net_id + 1];
             ++net2pin_id) {
            int net_pin_id = flat_net2pin_map[net2pin_id];
            int other_node_id = pin2node_map[net_pin_id];

            if (((other_node_id < num_movable_nodes) && (node_weight[other_node_id] == c_id)) ||
                ((other_node_id >= num_movable_nodes) && (node_weight[other_node_id] == 1))) {
                node_exist_flag = true;

                boxs[c_id].xl = std::min(boxs[c_id].xl, x[other_node_id] + pin_offset_xs[c_id][net_pin_id]);
                boxs[c_id].xh = std::max(boxs[c_id].xh, x[other_node_id] + pin_offset_xs[c_id][net_pin_id]);
                boxs[c_id].yl = std::min(boxs[c_id].yl, y[other_node_id] + pin_offset_ys[c_id][net_pin_id]);
                boxs[c_id].yh = std::max(boxs[c_id].yh, y[other_node_id] + pin_offset_ys[c_id][net_pin_id]);
            }
        }
        if (!node_exist_flag) {
            boxs[c_id].xl = 0;
            boxs[c_id].xh = 0;
            boxs[c_id].yl = 0;
            boxs[c_id].yh = 0;
        }
    }

    partial_hpwl[net_id][0] = boxs[0].xh - boxs[0].xl + boxs[1].xh - boxs[1].xl;
    partial_hpwl[net_id][1] = boxs[0].yh - boxs[0].yl + boxs[1].yh - boxs[1].yl;
    return (boxs[0].xh - boxs[0].xl + boxs[1].xh - boxs[1].xl) + (boxs[0].yh - boxs[0].yl + boxs[1].yh - boxs[1].yl);
}  // END MODULE

//---------------------------------------------------------------------

Box DetailedPlaceData::compute_optimal_region(int node_id) const {
    Box box(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(), -std::numeric_limits<float>::max(),
            -std::numeric_limits<float>::max());
    for (int node2pin_id = flat_node2pin_start_map[node_id]; node2pin_id < flat_node2pin_start_map[node_id + 1];
         ++node2pin_id) {
        int node_pin_id = flat_node2pin_map[node2pin_id];
        int net_id = pin2net_map[node_pin_id];
        if (net_mask[net_id]) {
            for (int net2pin_id = flat_net2pin_start_map[net_id]; net2pin_id < flat_net2pin_start_map[net_id + 1];
                 ++net2pin_id) {
                int net_pin_id = flat_net2pin_map[net2pin_id];
                int other_node_id = pin2node_map[net_pin_id];
                if ((node_id != other_node_id) && other_node_id < num_movable_nodes) {
                    box.xl = std::min(box.xl, x[other_node_id] + pin_offset_x[net_pin_id]);
                    box.xh = std::max(box.xh, x[other_node_id] + pin_offset_x[net_pin_id]);
                    box.yl = std::min(box.yl, y[other_node_id] + pin_offset_y[net_pin_id]);
                    box.yh = std::max(box.yh, y[other_node_id] + pin_offset_y[net_pin_id]);
                }
            }
        }
    }
    shift_box_to_layout(box);

    return box;
}  // END MODULE

//--------------------------------------------------------------------

float DetailedPlaceData::compute_total_hpwl_concurrent_c(const torch::TensorAccessor<float, 1> x,
                                                         const torch::TensorAccessor<float, 1> y,
                                                         float* net_hpwls) {
    /* add center pos info */
    pin_pos = pin_rel_cpos.clone();
    auto partial_hpwl_kernel = torch::zeros({2, num_nets, 2}, torch::dtype(pin_pos.dtype()));
    partial_hpwl = torch::zeros({num_nets, 2}, torch::dtype(torch::kFloat));
    pin_die = torch::zeros(flat_net2pin_map.size(0), torch::dtype(torch::kInt));

    // #pragma omp parallel for num_threads(num_threads)
    for (int i = 0; i < flat_net2pin_map.size(0); ++i) {
        int64_t node_id = pin_id2node_id[i].item<int>();
        // pin_pos[i][0] += init_x[node_id];
        // pin_pos[i][1] += init_y[node_id];

        // if (node_id == a || node_id == b) {
        //     cout << node_id << " at " <<  x[node_id] + node_size_x[node_id] / 2 << " , " << y[node_id] +
        //     node_size_y[node_id] / 2 << endl;
        // }

        pin_pos[i][0] += x[node_id] + node_size_x[node_id] / 2;
        pin_pos[i][1] += y[node_id] + node_size_y[node_id] / 2;

        pin_die[i] = node_weight[node_id];
        if ((node_id >= num_movable_nodes) && (node_weight[node_id] == 1)) pin_die[i] = 2;
        if ((node_id >= num_movable_nodes) && (node_weight[node_id] == 0)) pin_die[i] = -1;
    }
    pin_pos = torch::_cast_Int(pin_pos);

    int hpwl_int = 0;
    float hpwl_float_from_int = 0;
    float hpwl_float = 0;
    double hpwl_double = 0;

    // float hpwl = 0;
    // float hpwl_acc = 0;
    // float hpwl_ker_tensor_acc = 0;
    // #pragma omp parallel for num_threads(num_threads)
    for (int i = 0; i < num_nets; ++i) {
        net_hpwls[i] = compute_net_hpwl_c(i);

        for (int c = 0; c < 2; ++c) {  // FIXME: channel index
            int64_t start_idx = flat_net2pin_start_map[i];
            int64_t end_idx = flat_net2pin_start_map[i + 1];

            int64_t bot_count = 0;
            int64_t top_count = 0;
            int x_min_bot = 0;
            int x_max_bot = 0;
            int x_min_top = 0;
            int x_max_top = 0;

            for (int64_t idx = start_idx; idx < end_idx; idx++) {
                int64_t pin_id = flat_net2pin_map[idx];

                if (pin_die[pin_id].item<int>() == 0 || pin_die[pin_id].item<int>() == 2) {
                    if (bot_count == 0) {
                        x_min_bot = pin_pos[pin_id][c].item<int>();
                        x_max_bot = pin_pos[pin_id][c].item<int>();
                    } else {
                        int xx = pin_pos[pin_id][c].item<int>();
                        x_min_bot = min(xx, x_min_bot);
                        x_max_bot = max(xx, x_max_bot);
                    }
                    bot_count++;
                }
                if (pin_die[pin_id].item<int>() == 1 ||
                    pin_die[pin_id].item<int>() == 2) {  // FIXME: fix bug: if <- else if
                    if (top_count == 0) {
                        x_min_top = pin_pos[pin_id][c].item<int>();
                        x_max_top = pin_pos[pin_id][c].item<int>();
                    } else {
                        int xx = pin_pos[pin_id][c].item<int>();
                        x_min_top = min(xx, x_min_top);
                        x_max_top = max(xx, x_max_top);
                    }
                    top_count++;
                }
            }
            partial_hpwl_kernel[0][i][c] = abs(x_max_bot - x_min_bot);
            partial_hpwl_kernel[1][i][c] = abs(x_max_top - x_min_top);

            hpwl_int += abs(x_max_bot - x_min_bot) + abs(x_max_top - x_min_top);
            hpwl_float_from_int += (int)(abs(x_max_bot - x_min_bot) + abs(x_max_top - x_min_top));
            hpwl_float += abs(x_max_bot - x_min_bot) + abs(x_max_top - x_min_top);
            hpwl_double += abs(x_max_bot - x_min_bot) + abs(x_max_top - x_min_top);

            // cout << abs(x_max_bot - x_min_bot) + abs(x_max_top - x_min_top) << endl;
            // hpwl_ker_tensor_acc += (partial_hpwl_kernel[0][i][c] + partial_hpwl_kernel[1][i][c]).item<float>();
        }

        // hpwl_acc += (partial_hpwl_kernel[0][i][0] + partial_hpwl_kernel[0][i][1] + partial_hpwl_kernel[1][i][0] +
        //              partial_hpwl_kernel[1][i][1])
        //                 .item<float>();

        // hpwl_acc += (partial_hpwl_kernel[0][i][0] + partial_hpwl_kernel[0][i][1]).item<float>();
        // hpwl_acc += (partial_hpwl_kernel[1][i][0] + partial_hpwl_kernel[1][i][1]).item<float>();

        // hpwl_acc += (partial_hpwl_kernel[0][i][0] + partial_hpwl_kernel[1][i][0]).item<float>();
        // hpwl_acc += (partial_hpwl_kernel[0][i][1] + partial_hpwl_kernel[1][i][1]).item<float>();

        // cout << "Net " << i << " at 0 " << (partial_hpwl_kernel[0][i][0] + partial_hpwl_kernel[0][i][1]).item<int>()
        //      << endl;

        // cout << "Net " << i << " at 1 " << (partial_hpwl_kernel[1][i][0] + partial_hpwl_kernel[1][i][1]).item<int>()
        //      << endl;
    }

    int total_hpwl = (torch::sum((partial_hpwl_kernel.index({0, "..."})).sum(1)) +
                      torch::sum((partial_hpwl_kernel.index({1, "..."})).sum(1)))
                         .item<int>();

    logger.info("Kernel INT ACC HPWL %d", hpwl_int);
    logger.info("Kernel FLOAT FROM INT ACC HPWL %.3f", hpwl_float_from_int);
    logger.info("Kernel FLOAT ACC HPWL %.3f", hpwl_float);
    logger.info("Kernel DOUBLE ACC HPWL %.3f", hpwl_double);

    // printf("Kernel ACC Tensor HPWL %.3f\n", hpwl_ker_tensor_acc);
    // printf("Kernel HPWL %d\n", total_hpwl);

    // I found OpenMP reduction cannot guarantee run-to-run determinism
    //#pragma omp parallel for num_threads(state.num_threads) default(shared)
    // for (int i = 0; i < num_nets; ++i) {
    //     hpwl += net_hpwls[i];

    //     // hpwl_acc += (partial_hpwl_kernel[0][i][0] + partial_hpwl_kernel[0][i][1] + partial_hpwl_kernel[1][i][0] +
    //     //              partial_hpwl_kernel[1][i][1])
    //     //                 .item<float>();
    //     // cout << partial_hpwl_kernel[0][i][0] + partial_hpwl_kernel[0][i][1] + partial_hpwl_kernel[1][i][0] +
    //     //             partial_hpwl_kernel[1][i][1]
    //     //      << endl;
    //     // cout << partial_hpwl[i][0] + partial_hpwl[i][1] << endl;
    //     // cout << net_hpwls[i] << endl;
    // }

    // printf("Acc HPWL %.3f\n", hpwl_acc);
    // printf("Merged HPWL hpwl %.3f\n", hpwl);

    return float(hpwl_int);
}  // END MODULE

float DetailedPlaceData::compute_net_hpwl_c(int net_id) const {
    vector<Box> boxs;
    boxs.emplace_back(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                      -std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());
    boxs.emplace_back(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                      -std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());

    vector<Box> boys;
    boys.emplace_back(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                      -std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());
    boys.emplace_back(std::numeric_limits<float>::max(), std::numeric_limits<float>::max(),
                      -std::numeric_limits<float>::max(), -std::numeric_limits<float>::max());

    int64_t start_idx = flat_net2pin_start_map[net_id];
    int64_t end_idx = flat_net2pin_start_map[net_id + 1];

    int64_t bot_count = 0;
    int64_t top_count = 0;
    float x_min_bot = 0;
    float x_max_bot = 0;
    float x_min_top = 0;
    float x_max_top = 0;

    float y_min_bot = 0;
    float y_max_bot = 0;
    float y_min_top = 0;
    float y_max_top = 0;

    for (int net2pin_id = flat_net2pin_start_map[net_id]; net2pin_id < flat_net2pin_start_map[net_id + 1];
         ++net2pin_id) {
        int net_pin_id = flat_net2pin_map[net2pin_id];
        int other_node_id = pin2node_map[net_pin_id];

        if (pin_die[net_pin_id].item<int>() == 0 || pin_die[net_pin_id].item<int>() == 2) {
            if (bot_count == 0) {
                x_min_bot = pin_pos[net_pin_id][0].item<float>();
                x_max_bot = pin_pos[net_pin_id][0].item<float>();

                y_min_bot = pin_pos[net_pin_id][1].item<float>();
                y_max_bot = pin_pos[net_pin_id][1].item<float>();
            } else {
                float xx = pin_pos[net_pin_id][0].item<float>();
                x_min_bot = min(xx, x_min_bot);
                x_max_bot = max(xx, x_max_bot);

                float yy = pin_pos[net_pin_id][1].item<float>();
                y_min_bot = min(yy, y_min_bot);
                y_max_bot = max(yy, y_max_bot);
            }
            bot_count++;
        }

        if (pin_die[net_pin_id].item<int>() == 1 || pin_die[net_pin_id].item<int>() == 2) {
            if (top_count == 0) {
                x_min_top = pin_pos[net_pin_id][0].item<float>();
                x_max_top = pin_pos[net_pin_id][0].item<float>();

                y_min_top = pin_pos[net_pin_id][1].item<float>();
                y_max_top = pin_pos[net_pin_id][1].item<float>();
            } else {
                float xx = pin_pos[net_pin_id][0].item<float>();
                x_min_top = min(xx, x_min_top);
                x_max_top = max(xx, x_max_top);

                float yy = pin_pos[net_pin_id][1].item<float>();
                y_min_top = min(yy, y_min_top);
                y_max_top = max(yy, y_max_top);
            }
            top_count++;
        }
    }

    return abs(x_max_bot - x_min_bot) + abs(x_max_top - x_min_top) + abs(y_max_bot - y_min_bot) +
           abs(y_max_top - y_min_top);

}  // END MODULE

}  // namespace dpc